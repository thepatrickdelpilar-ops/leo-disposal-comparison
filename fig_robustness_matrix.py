...
def main():
    ...
    print(f"IADC unachievable (censored) in {censored} combinations; FCC achievable in all")
    print(f"finite gaps (n={len(gaps)}): median {np.median(gaps):.1f}, range [{gaps.min():.1f}, {gaps.max():.1f}]")
    ...
    ax.set_xlabel("IADC - FCC compliance gap (pts), finite cases"); ax.set_ylabel("Parameter combinations")
